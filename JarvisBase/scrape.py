...
def load_vectors_into_deeplake(dataset_path, source_chunks):
    ...
    deeplake_db = DeepLake(dataset_path=dataset_path, embedding_function=embeddings,token=active_loop_token)
...
def main():
    ...
    os.remove(filename)
    documents = []
    for file in os.listdir("JarvisBase/docs"):
        if file.endswith('.pdf'):
            pdf_path = 'JarvisBase/docs/' + file
            loader = PyPDFLoader(pdf_path)
            documents.extend(loader.load())
        elif file.endswith('.docx') or file.endswith('.doc'):
            doc_path = 'JarvisBase/docs/' + file
            loader = Docx2txtLoader(doc_path)
            documents.extend(loader.load())
        elif file.endswith('.txt'):
            text_path = 'JarvisBase/docs/' + file
