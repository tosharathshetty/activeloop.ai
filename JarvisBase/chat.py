...
def search_db(user_input, db):
    ...
    model = ChatOpenAI(model_name="gpt-3.5-turbo",
                       ...
                       verbose=True,
                        max_tokens=500,)  # switch to 'gpt-4'
    qa = ConversationalRetrievalChain.from_llm(model, retriever=retriever,max_tokens_limit=1000)
    # qa = RetrievalQA.from_llm(model, retriever=retriever, return_source_documents=True, max_tokens_limit=4096)
    
    # qa = ConversationalRetrievalChain.from_llm(
    #     OpenAI(temperature=0), retriever=retriever, memory=memory
    # )
    return qa({'question': user_input, "chat_history": chat_history})
...
def main():
    ...
    st.write("# Insurance document search :sunglasses:")
